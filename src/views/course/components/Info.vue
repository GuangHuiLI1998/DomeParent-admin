<template>
  <div class="app-container">
    ...
    <div style="text-align:center">
      <el-button :disabled="saveBtnDisabled" type="primary" plain @click="saveAndNext()">保存并下一步</el-button>
    </div>
  ...
  methods: {
    ...
    subjectChange(value) {
      ...
      this.subjectList.forEach(subject => {
        // 判断当前选中的一级类别是否和当前便利的一级类别id一致
        if (subject.id === value) {
          // 清空当前二级的值
          this.courseInfo.subjectId = ''
          // 如果一致，则将当前遍历的一级类别的子类别绑定在页面的二级类别表中
    ...
    saveData() {
      courseApi.saveCourseInfo(this.courseInfo).then(response => {
        ...
        this.$parent.active = 1 // 下一步 访问父组件成员 $parent
      })
    },
    ...
    fetchCourseInfoById(id) {
      courseApi.getCourseInfoById(id).then(response => {
        ...
        subjectApi.getNestedTreeList().then(response => {
          ...
          // 判断this.subjectList 下那个一级类别是当前绑定的一级类别
          this.subjectList.forEach(subject => {
